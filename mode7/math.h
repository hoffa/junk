...
#define NUM_TO_FIX(a) ((a) << 16)
#define FLOAT_TO_FIX(a) ((Sint32)((a) * 65536))
...
vec2_t* addVec2(vec2_t* a, vec2_t* b) {
  a->x += b->x;
  a->y += b->y;
  return a;
}
...
static __inline__ __attribute__((always_inline))
vec2_t* subVec2(vec2_t* a, vec2_t* b) {
  a->x -= b->x;
  a->y -= b->y;
  return a;
}

#define FIX_MUL(a, b) ((Sint32)(((Sint64)(a) * (Sint64)(b)) >> 16))

static __inline__ __attribute__((always_inline))
vec2_t* rotateVec2(vec2_t* a, Sint32 sine, Sint32 cosine) {
  vec2_t tmp = *a;
  a->x = FIX_MUL(tmp.x, cosine) - FIX_MUL(tmp.y, sine);
  a->y = FIX_MUL(tmp.x, sine) + FIX_MUL(tmp.y, cosine);
  return a;
}

static __inline__ __attribute__((always_inline))
Sint32 sinLut(Sint32 a) {
  switch ((a & 0x30000) >> 16) {
    case 0: return sineTable[((a >> 8) & 0xFF)];
    case 1: return sineTable[256 - ((a >> 8) & 0xFF)];
    case 2: return -sineTable[((a >> 8) & 0xFF)];
    case 3: return -sineTable[256 - ((a >> 8) & 0xFF)];
    default: return 0;
  }
}

static __inline__ __attribute__((always_inline))
Sint32 cosLut(Sint32 a) {
  return sinLut(a - 0x10000);
}
